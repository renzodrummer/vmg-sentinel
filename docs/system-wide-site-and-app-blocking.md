...
**Shipped Windows behavior** (helper `fw-5`, see [`work-session-blocking-full-documentation.md`](./work-session-blocking-full-documentation.md) and [`work-session-site-and-app-blocking.md`](./work-session-site-and-app-blocking.md)):

- **Sites:** Windows Firewall outbound blocks of DNS-resolved deny-list IPs (not WFP SNI, not hosts file).
- **Apps:** session-gated AppLocker **Exe** Allow `%WINDIR%\*` / `%PROGRAMFILES%\*` / Everyone `*` plus **Exe Deny** (path, SHA-256, or publisher); **Appx** allow all signed packaged apps (Settings / This PC → Properties); and a **one-shot** close of already-running matches. Deny-only Enabled AppLocker (`fw-3`) is rejected. Exe-only without Appx (`fw-4`) is rejected — it blocks Settings. **WDAC XML is never deployed**.
- **Reconcile:** 45s DNS refresh; Firewall/AppLocker rewrite is skipped when the deny lists and resolved IPs are unchanged.
- Rules are lifted on **Stop Tracking**, **Quit** (`SetSession(false)`), helper exit, `--clear-blocks`, and `--uninstall` (Exe **and Appx** NotConfigured).
- `npm run dist` runs `verify:helper` and will not package a pre-`fw-5` helper or one missing the Exe/Windows/packaged allow strings.
...
| 2026-09-25 | Helper `fw-5`: Appx allow all signed packaged apps so Settings / This PC → Properties stay usable; Exe also allows `%WINDIR%\*` and `%PROGRAMFILES%\*` (no Administrators allow). Clear sets Exe and Appx NotConfigured. Electron rejects `fw-4`. Dist verifies packaged-allow and Windows-allow strings. |
